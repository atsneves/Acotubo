...
create: function () {
    ...
    div.innerHTML = '<div id="sw-header"><div id="sw-cancel">Voltar</' + 'div><div id="sw-done">Cotar</' + 'div></' + 'div><div id="sw-slots-wrapper"><div id="sw-slots"></' + 'div></' + 'div><div id="sw-frame"></' + 'div>';
    ...
    for (l = 0; l < this.slotData.length; l += 1) {
        ...
        for (i in this.slotData[l].values) {
            var ide = i.replace(".","_");
            if (ide.split("|").length > 2)
            {
                var ide = "Tubo"+ide.split("|")[2];
            }
            
            out += '<li id="slot'+ ide +'" valorSlot="'+i+'">' + this.slotData[l].values[i] + '<' + '/li>';
        }
        ul.innerHTML = out;
        ul.id = "vlUl"+l;
...
open: function () {
    ...
    valores = this.getSelectedValues();
    
    document.getElementById("conteudoPicker").value=valores.keys;
    document.getElementById("conteudoPickerDescr").value=valores.values;
    Calculos();
},
...
scrollEnd: function (e) {
    ...
    var valo = Math.round(this.slotEl[this.activeSlot].slotYPosition / this.cellHeight) * this.cellHeight;
    ...
    if (scrollDistance < this.cellHeight / 1.5 && scrollDistance > -this.cellHeight / 1.5) {
        if (this.slotEl[this.activeSlot].slotYPosition % this.cellHeight) {
            this.scrollTo(this.activeSlot, valo, '100ms');
        }
        valores = this.getSelectedValues();
        ...
        for (x = 0;x < ks.length;x++) {
            if (ks[x] == "")
            {   
                this.scrollToValue(x, this.slotData[x].defaultValue);
                valores = this.getSelectedValues();
